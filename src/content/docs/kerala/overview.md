---
title: Eating in Kerala
description: A short orientation to Kerala's table — the banana leaf, eating by hand, and the communities whose customs shape a meal.
sidebar:
  order: 1
  label: Overview
lastUpdated: 2026-05-25
---

A meal in Kerala is built around **rice**, eaten with the **right hand**, and — at its most formal — served on a **banana leaf** (*ela*). Whether it's a weekday lunch of rice and fish curry or a grand festival *sadya*, a few shared habits run through almost every table: food is served to you rather than taken by you, elders and guests eat first, and the host's job is to keep your leaf full.

This guide breaks Kerala dining into practical chapters. If you're a visitor or simply unsure, start with **[The Sadya](/kerala/sadya/)** and **[Eating by Hand](/kerala/hand-eating/)**, then skim **[Serving & Hosting](/kerala/serving-and-hosting/)** so you know how to politely say *"enough"*.

## One state, many tables

There is no single "Kerala way" to eat. Customs shift with community and occasion:

- **Hindu households** keep the festival *sadya* strictly vegetarian, served on a leaf.
- **Syrian Christian** tables are known for meat — beef, duck, and pork dishes — alongside appam and stew, especially at weddings and Christmas.
- **Muslim (Mappila) communities** of the Malabar coast bring biryani, *pathiri*, and the *kanji* and *nombu* foods of Ramadan, and sometimes a tradition of several people sharing one large platter.

:::note[🧭 Regional variation]
We note these differences throughout rather than presenting one community's practice as "Kerala's". When a custom is specific to a group or occasion, the page will say so.
:::

## How to use these guides

Each page describes what people *do*, not a set of commandments. Where a tradition carries a genuine health risk — a shared water tumbler, double-dipping into a common dish — you'll find a **hygiene note** that names the risk and suggests a gentler, safer alternative. The aim is respect for the custom *and* care for the people at the table.

## Sources & further reading

- [Kerala cuisine — Wikipedia](https://en.wikipedia.org/wiki/Kerala_cuisine)
- [Sadya — Wikipedia](https://en.wikipedia.org/wiki/Sadya)
